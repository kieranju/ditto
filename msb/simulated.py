...
from msb.operator.definitions import MButton, KKey, key_dictionary
...
def m_click(button: MButton, duration=0.15, variance_limit=0.015):
    operator.m_press(button)
    if variance_limit > 0 or duration > 0:
        variance = 0 if variance_limit == 0 else random.uniform(-variance_limit, variance_limit)
        delay = perf_counter() + (duration + variance)
        while perf_counter() < delay: pass
    operator.m_release(button)

def m_hit(button: MButton):
    m_click(button, 0, 0)
...
def k_click(key: KKey, duration=0.15, variance_limit=0.015):
    operator.k_press(key)
    if variance_limit > 0 or duration > 0:
        variance = 0 if variance_limit == 0 else random.uniform(-variance_limit, variance_limit)
        delay = perf_counter() + (duration + variance)
...
def k_lookup(char):
    return key_dictionary.get(char)

# TODO: add ability to make mistakes and delete them, then continue
def k_type(phrase, do_make_mistakes=True, words_per_minute=125):
    ...
    time_correction = (word_delay_upper_limit / average_word_size) * word_count  # accommodate for our induced randomness
